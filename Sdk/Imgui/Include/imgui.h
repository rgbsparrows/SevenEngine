...
namespace ImGui
{
    ...
    IMGUI_API bool          IsWindowActive(const char* str_id);         // is window active
    IMGUI_API ImDrawList*   GetWindowDrawList();                        // get draw list associated to the current window, to append your own drawing primitives
    IMGUI_API float         GetWindowDpiScale();                        // get DPI scale currently associated to the current window's viewport.
	IMGUI_API ImVec2        GetWindowPos(const char* str_id);           // get current window position in screen space (useful if you want to do your own drawing via the DrawList API)
	IMGUI_API ImVec2        GetWindowPos();                             // get current window position in screen space (useful if you want to do your own drawing via the DrawList API)
	IMGUI_API ImVec2        GetWindowSize(const char* str_id);          // get current window size
    ...
    IMGUI_API bool          InputText(const char* label, std::string& inoutStr, size_t buf_size, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_MainWindow             = 1 << 22,  // Use Main Viewport Size As Init Size
