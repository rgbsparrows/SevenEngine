...
namespace UnrealBuildTool
{
	...
	static class UnrealBuildTool
	{
		...
		/// </summary>
		public static readonly DirectoryReference EngineDirectory = RootDirectory;
			...
			bool bCreatedMutex = false;
			{
				...
				try
				{
					...
					foreach (string Arg in Arguments)
					{
						...
						{
							// May be for compiling; don't set bGenerateProjectFiles by default, but do override the compiler if it is. 
							ProjectFileFormats.Add(ProjectFileFormat.VisualStudio2019);
							OverrideWindowsCompiler = WindowsCompiler.VisualStudio2019;
						}
						else if (LowercaseArg == "development" || LowercaseArg == "debug" || LowercaseArg == "shipping" || LowercaseArg == "test")
						{
							//ConfigName = Arg;
						}
						...
						foreach (ProjectFileFormat ProjectFileFormat in ProjectFileFormats.Distinct())
						{
							...
							switch (ProjectFileFormat)
							{
								...
								case ProjectFileFormat.VisualStudio2019:
									Generator = new VCProjectFileGenerator(ProjectFile, VCProjectFileFormat.VisualStudio2019, OverrideWindowsCompiler);
									break;
								default:
				...
				using (ISourceFileWorkingSet WorkingSet = SourceFileWorkingSet.Create(UnrealBuildTool.RootDirectory, DirectoryReference.FromFile(ProjectFile)))
				{
					...
					foreach (UEBuildTarget Target in Targets)
					{
						...
						if (bIsGatheringBuild)
						{
							...
							{
								BuildResult = Target.Clean();
							}
							else
							{
								BuildResult = Target.Build(BuildConfiguration, TargetToHeaders[Target], TargetOutputItems, WorkingSet, ActionGraph, HotReload, bIsAssemblingBuild);
							}
