...
namespace UnrealBuildTool
{
    ...
    class UEBuildTarget : ISerializable
    {
        ...
        public static UEBuildTarget CreateTarget(TargetDescriptor Desc, string[] Arguments, bool bCompilingSingleFile, ReadOnlyBuildVersion Version)
        {
            ...
            {
                if (RulesObject.Type != TargetType.Program && (UnrealBuildTool.IsEngineInstalled()))
                {
        ...
        static void ValidateSharedEnvironment(RulesAssembly RulesAssembly, string ThisTargetName, TargetRules ThisRules)
        {
            ...
            switch (ThisRules.Type)
            {
                case TargetType.Game:
                    BaseTargetName = "SevenGame";
                    break;
                case TargetType.Editor:
                    BaseTargetName = "SevenEditor";
                    break;
                case TargetType.Client:
                    BaseTargetName = "SevenClient";
                    break;
                case TargetType.Server:
                    BaseTargetName = "SevenServer";
                    break;
                default:
                    return;
            }
        ...
        public UEBuildTarget(TargetDescriptor InDesc, ReadOnlyTargetRules InRules, RulesAssembly InRulesAssembly, FileReference InTargetCsFilename)
        {
            ...
            DirectoryReference OutputDirectory = ProjectDirectory;

            bool bCompileAsDLL = Rules.bShouldCompileAsDLL;

            OutputPaths = MakeBinaryPaths(OutputDirectory, TargetName, Platform, Configuration, bCompileAsDLL ? UEBuildBinaryType.DynamicLinkLibrary : UEBuildBinaryType.Executable, Rules.Architecture, Rules.UndecoratedConfiguration, ProjectFile != null, Rules.ExeBinariesSubFolder, ProjectFile, Rules);
        }
        ...
        public ECompilationResult Clean()
        {
        ...
        public void WriteReceipts()
        {
            ...
            {
                FileReference pluginManifestFile = FileReference.Combine(ProjectDirectory, "Binaries", Platform.ToString(), PluginManifest.GetStandardFileName(AppName, Platform, Configuration, Architecture));
                if (!IsFileInstalled(pluginManifestFile))
                {
                    new PluginManifest(ProjectDirectory).Write(pluginManifestFile.FullName);
                }
            }
        }
        ...
        /// </summary>
        public ECompilationResult Build(BuildConfiguration BuildConfiguration, CPPHeaders Headers, List<FileItem> OutputItems, ISourceFileWorkingSet WorkingSet, ActionGraph ActionGraph, EHotReload HotReload, bool bIsAssemblingBuild)
        {
            ...
            // Add global definitions for project-specific binaries. HACK: Also defining for monolithic builds in binary releases. Might be better to set this via command line instead?
            {
                ...
                bool IsCurrentPlatform = Platform == UnrealTargetPlatform.Win64 || Platform == UnrealTargetPlatform.Win32;
                    ...
                    && IsCurrentPlatform)
                {
                    ...
                    GlobalCompileEnvironment.Definitions.Add(String.Format("SE_ENGINE_DIRECTORY=\"{0}\"", EnginePath));
                }
            }
            ...
            GlobalLinkEnvironment.bShouldCompileMonolithic = true;
        ...
        private string GetModulePackageName(string ModuleName, PluginInfo Plugin)
        {
            if(Plugin != null)
            {
                return Plugin.Name;
            }
            else if(RulesAssembly.IsGameModule(ModuleName))
            {
                return "Game";
            }
            else
        ...
        private DirectoryReference GetModuleIntermediateDirectory(string ModuleName, ModuleRules ModuleRules, PluginInfo Plugin)
        {
            ...
            // Get the configuration that this module will be built in.
        ...
        private void SetupGlobalEnvironment(UEToolChain ToolChain, CppCompileEnvironment GlobalCompileEnvironment, LinkEnvironment GlobalLinkEnvironment)
        {
            ...
            GlobalLinkEnvironment.bHasExports = false;
            ...
            UnrealTargetConfiguration EngineTargetConfiguration = Configuration;
            ...
            {
                GlobalCompileEnvironment.Definitions.Add("SE_BUILD_MINIMAL=1");
            }
            else
            {
                GlobalCompileEnvironment.Definitions.Add("SE_BUILD_MINIMAL=0");
            }
            ...
            if (Rules.bCompileCEF3 && (Platform == UnrealTargetPlatform.Win32 || Platform == UnrealTargetPlatform.Win64))
            {
            ...
            GlobalCompileEnvironment.Definitions.Add(String.Format("SBT_PLUGIN_MANIFEST=\"{0}\"", PluginManifest.GetStandardFileName(AppName, Platform, Configuration, Architecture)));

            // tell the compiled code the name of the UBT platform (this affects folder on disk, etc that the game may need to know)
            GlobalCompileEnvironment.Definitions.Add("SBT_COMPILED_PLATFORM=" + Platform.ToString());
            GlobalCompileEnvironment.Definitions.Add("SBT_COMPILED_TARGET=" + TargetType.ToString());
        ...
        public UEBuildModule FindOrCreateModuleByName(string ModuleName, string ReferenceChain)
        {
            ...
            if (!Modules.TryGetValue(ModuleName, out Module))
            {
                ...
                Module = InstantiateModule(RulesObject, ModuleName, ModuleDirectory, GeneratedCodeDirectory, FoundSourceFiles, bBuildFiles, ModuleFileName, RuntimeDependencies, Plugin);
            ...
            switch (RulesObject.Type)
            {
                case ModuleRules.ModuleType.CPlusPlus:
                    return new UEBuildModuleCPP(
                            InName: ModuleName,
                            InPackageName: GetModulePackageName(ModuleName, Plugin),
